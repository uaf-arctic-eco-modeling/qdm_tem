...
var = os.getenv('var')
era5startyr = os.getenv('era5startyr')

### CHECK THIS AND AUTOMATE!!!!
...
dd = pd.melt(dd, id_vars=['y','x'], value_vars=['Band' + str(s) for s in list(range(1,1025))])
dd['timestep'] = dd['variable'].str.extract(r'(\d+)').astype(int)
#dd[(dd['x'] == -1218000.0) & (dd['y'] == 2398000.0)].head(50)
...
#dd['time'] = ((int(era5startyr)-1901)*365 + (np.floor(dd['timestep']/12)) * 365 + dd['doy']).astype(int)
dd['year'] = (np.floor((dd['timestep'] - 1)/12)+1940).astype(int)
dd['time'] = ((dd['year']-1901)*365) - 1 + dd['doy_noleap']
#dd['day'] = 1
#dd['time'] = pd.to_datetime(dd[['year', 'month', 'day']])
...
#nc['time'] = nc['time'].astype('datetime64[ns]')
...
nc['time'].attrs={'units':'days since 1901-1-1 0:0:0','long_name':'time','calendar':'365_day'}
...
nc.attrs = ddo.attrs
#nc.to_netcdf('/Volumes/5TIV/PROCESSED/TILES2_0/H1_V10/climate_downscaling/ERA5/tmp_t2m2.nc',mode='w')
#nc.to_netcdf('/Volumes/5TIV/PROCESSED/TILES2_0/H1_V10/climate_downscaling/ERA5/tmp_t2m2.nc',unlimited_dims='time',mode='w')
