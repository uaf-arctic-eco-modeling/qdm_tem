...
dd = pd.melt(dd, id_vars=['y','x'], value_vars=['Band' + str(s) for s in list(range(1,13))])
dd['month'] = dd['variable'].str.extract(r'(\d+)').astype(int)
dd = pd.merge(dd,month_info, how='outer', on='month')
#if (int(year) % 4 == 0 and int(year) % 100 != 0) or (int(year) % 400 == 0):
dd['time'] = ((int(year)-1901)*365) - 1 + dd['doy_noleap']
dd = dd.rename(columns={'value': var, 'x': 'lon', 'y': 'lat'})
dd = dd[['time','lat','lon',var]]
nc = dd.set_index(['time', 'lat', 'lon']).to_xarray()
#nc.convert_calendar("noleap")
...
#ddm = dd.groupby(['time'])[[var]].mean()
#ddm.reset_index(inplace=True)
#plt.plot(ddm['time'], ddm[var])

#ddo1 = xr.open_dataset(resample_path)
