...
var = os.getenv('var')
cmiphiststart = os.getenv('cmiphiststart')
year = int(cmiphiststart[:4])
...
dd = pd.melt(dd, id_vars=['y','x'], value_vars=['Band' + str(s) for s in list(range(1,541))])
...
dd['year'] = (np.floor((dd['timestep'] - 1)/12)+1970).astype(int)
dd['time'] = ((dd['year']-1901)*365) - 1 + dd['doy_noleap']
#dd['year'] = np.floor(dd['timestep']/12)
#dd['time'] = ((year-1901)*365 + (np.floor(dd['timestep']/12)) * 365 + dd['doy']).astype(int)
